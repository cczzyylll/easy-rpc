package rpc.core.dispatcher;

import com.google.common.util.concurrent.ThreadFactoryBuilder;
import rpc.core.common.RpcInvocation;
import rpc.core.common.RpcProtocol;
import rpc.core.common.exception.RpcException;
import rpc.core.server.ServerChannelReadData;
                    ...
                    executorService.submit(new Runnable() {
                        ...
                        public void run() {
                            try {
                                ...
                                RpcInvocation rpcInvocation = CommonServerCache.SERVER_SERIALIZE_FACTORY.deserialize(rpcProtocol.getContent(), RpcInvocation.class);
                                //执行过滤链路
                                try {
                                    CommonServerCache.SERVER_BEFORE_FILTER_CHAIN.doFilter(rpcInvocation);
                                    ...
                                    if (cause instanceof RpcException) {
                                        ...
                                        byte[] body = CommonServerCache.SERVER_SERIALIZE_FACTORY.serialize(reqParam);
                                ...
                                CommonServerCache.SERVER_AFTER_FILTER_CHAIN.doFilter(rpcInvocation);
                                RpcProtocol respRpcProtocol = new RpcProtocol(CommonServerCache.SERVER_SERIALIZE_FACTORY.serialize(rpcInvocation));
                                serverChannelReadData.getChannelHandlerContext().writeAndFlush(respRpcProtocol);
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
